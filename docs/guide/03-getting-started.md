...
Both assume you have installed the Fractal CLI tool globally as described in the {{ linkTo('@installation', 'installation guide' )}}.
...
6. Use your browser to open the 'Local URL' displayed in your terminal to view your component library.

## B) The 'from scratch' method

The following steps will describe how to create a basic Fractal project from scratch.

It assumes that you have already created a new, empty directory for your project and have installed Fractal as a dependency in your project (as well as the global CLI tool). If not then first check out the {{ linkTo('@installation', 'installation guide' )}} for instructions.
...
Obviously this isn't a very exciting component at this point, but there is {{ linkTo('@components', 'plenty more you can do') }} once you've got to grips with the basics.
...
The Markdown file also contains some YAML front-matter to specify a title for the page - check out the {{ linkTo('@docs', 'documentation pages section') }} for more details on this and all the other things that you can do with Fractal's powerful documentation engine.
...
If so, visit the 'Local URL' (It should be something like http://localhost:3000) and have a look at your component library's web UI. It's a little empty

Note that using the `--sync` option here instructs Fractal to use BrowserSync to seamlessly watch your file system for changes and refresh the web UI when you make updates to your files. You can read the {{ linkTo('@web', 'web UI docs') }} for more details on the options available when starting up development servers, as well as things like how to run static HTML exports of your component library UI.
